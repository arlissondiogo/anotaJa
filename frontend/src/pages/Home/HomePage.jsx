  ...
  return (
    <div className="home-page">
        ...
        <Modal onClose={() => setOrderModal(null)}>
          <p className="modal-title">
            {isDelivery(orderModal.table)
              ? "🛵 Delivery"
              : `Mesa ${orderModal.table.number}`}
          </p>
          <p className="modal-subtitle">
            Cliente: {orderModal.table.clientName}
          </p>
          ...
          {orderModal.order && orderModal.order.items?.length > 0 && (
            <div className="order-items">
              ...
              <div className="order-total">
                Total: R$ {orderModal.order.total?.toFixed(2)}
              </div>
            </div>
          )}
          ...
          <div className="modal-actions">
            {!orderModal.order && (
              <>
                <button
                  className="btn btn--gray btn--md"
                  onClick={() => handleFreeTable(orderModal.table.id)}
                >
                  {isDelivery(orderModal.table)
                    ? "Cancelar delivery"
                    : "Liberar mesa"}
            ...
            {orderModal.order && (
              <>
                <button
                  className="btn btn--gray btn--md"
                  onClick={() =>
                    handleOpenEdit(orderModal.order, orderModal.table)
                  }
                >
                  Editar pedido
                </button>
                <button
                  ...
                  onClick={() => setCancelConfirm(orderModal.order.id)}
                >
                  Cancelar pedido
                </button>
                <button
                  ...
                  onClick={() =>
                    handlePay(orderModal.order.id, orderModal.table)
                  }
                >
                  Pagar
                </button>
      ...
      {menuModal && (
        ...
        >
          <p className="modal-title">
            {isDelivery(menuModal)
              ? "🛵 Cardápio — Delivery"
              : `Cardápio — Mesa ${menuModal.number}`}
          </p>
          ...
          {cartCount > 0 && (
            <div className="cart-summary">
              {cartCount} {cartCount === 1 ? "item" : "itens"} · R${" "}
              {cartTotal.toFixed(2)}
            </div>
          )}
      ...
      {editModal && (
        ...
        >
          <p className="modal-title">
            {isDelivery(editModal.table)
              ? "🛵 Editar Pedido — Delivery"
              : `Editar Pedido — Mesa ${editModal.table.number}`}
          </p>
          <p className="modal-subtitle">
            Cliente: {editModal.table.clientName}
          </p>
